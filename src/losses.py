...
if __name__ == "__main__":
    ...
    # Simuler les sorties des modèles
    student_logits   = torch.randn(B, 1, H, W)
    targets          = torch.randint(0, 2, (B, 1, H, W)).float()

    # Simuler les feature maps (4 niveaux)
    teacher_features = [
        torch.randn(B,  64, H,      W),
        torch.randn(B, 128, H // 2, W // 2),
        torch.randn(B, 256, H // 4, W // 4),
        torch.randn(B, 512, H // 8, W // 8),
    ]
    student_adapted_features = [
        torch.randn(B,  64, H,      W),
        torch.randn(B, 128, H // 2, W // 2),
    ...
    # Test SegmentationLoss
    seg_loss = SegmentationLoss()
    l_seg = seg_loss(student_logits, targets)
    print(f"SegmentationLoss  : {l_seg.item():.4f}")

    # Test FeatureKDLoss
    kd_loss = FeatureKDLoss()
    l_kd = kd_loss(student_adapted_features, teacher_features)
    print(f"FeatureKDLoss     : {l_kd.item():.4f}")

    # Test TotalDistillationLoss
    total_loss = TotalDistillationLoss(lambda_kd=1.0)
    total, l_seg, l_kd = total_loss(
        student_logits, targets,
        student_adapted_features, teacher_features
    )
    print(f"TotalLoss         : {total.item():.4f}  "
          f"(seg={l_seg.item():.4f}, kd={l_kd.item():.4f})")